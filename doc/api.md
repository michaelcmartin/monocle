...
Clients create one or more *subscribers* to receive various kinds of events. When something interesting happens, one event is created for every subscriber that has subscribed to that kind of thing happening. Subscribers created via the layer 0 calls may only subscribe to layer 0 events. Entities, in layer 3, are subscribers that carry additional state with them and may subscribe to the layer 3 game-world-level events as well as to layer-0 raw input events. Generally, layer 0 subscribers handle meta-level processing such as handling game pause or screen-resize commands, or operating menus.
...
The event enumeration. Other events will be defined in later layers.
...
```C
int mncl_events_pending(void);
MNCL_EVENT *mncl_event_next(void);
...
These are the event loop accessor functions. The pointer returned by `mncl_event_next` is *owned by Monocle*, and the client should neither free it nor permit it to live too long. This pointer will be invalidated by the next call to `mncl_event_pop`. Attempting to pop a Quit event has no effect.
...
```C
int mncl_alloc_subscriber(void);
void mncl_free_subscriber(int subscriber);
void mncl_subscribe(int subscriber, MNCL_EVENT_TYPE e, int subtype1, int subtype2);
```

Manages layer 0 subscribers. The `subscriber` argument is a handle returned by `mncl_alloc_subscriber`, the event type indicates what type of event you wish to be notified of, and the subsequent arguments let you narrow those down. These are filters on the events, and are equality tests against their relevant values in the event structure; key events use subtype1 to name a key to listen for, while joystick events may be filtered by stick and index. Mouse buttons are filtered by button; Quit and Mouse Move events are not filtered at all. Filters that an event cannot understand are ignored. A filter set to `-1` means "do not filter on this field", so, for instance, `mncl_subscribe(s, MNCL_EVENT_MOUSEBUTTONDOWN, -1, -1)` will receive events whenever any mouse button is clicked.
