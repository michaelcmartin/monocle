...
{
    MNCL_RAW *raw = mncl_raw_resource("shadow");
    if (raw) {
...
main(int argc, char **argv)
{
    ...
    mncl_load_resmap("rawtest.json");
    test_string("RAW");
    mncl_unload_resmap("rawtest.json");
    mncl_add_resource_zipfile("rawtest.zip");
    mncl_load_resmap("rawtest.json");
    test_string("ZIP");
    raw = mncl_raw_resource("rawtest");
    if (!raw) {
        printf ("Couldn't find test data!\n");
        ++errors;
    } else {
